...
def prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    operators = [
        col.replace("op_weight_", "")
        for col in df.columns
        if col.startswith("op_weight_")
    ]

    return df, operators


def melt_operator_data(
    df: pd.DataFrame, operators: list, metric: str = "weight"
) -> pd.DataFrame:
...
def plot_weights_by_dimension(df: pd.DataFrame, operators: list, output_dir: Path):
    ...
    summary = (
        melted.groupby(["dimension", "operator_short"])["value"].mean().reset_index()
    )
    ...
    ax.legend(
        ...
        labels=[f"{val}D" for val in pivot.columns],
        fontsize=7,
        columnspacing=0.5,
        handlelength=1,
        handleheight=1,
...
def plot_weights_by_category(df: pd.DataFrame, operators: list, output_dir: Path):
    ...
    summary = (
        melted.groupby(["category", "operator_short"])["value"].mean().reset_index()
    )
...
def plot_weights_heatmap_dimension(df: pd.DataFrame, operators: list, output_dir: Path):
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "weights_heatmap_dimension.pdf", dpi=DPI, bbox_inches="tight"
    )
    plt.savefig(
        output_dir / "weights_heatmap_dimension.png", dpi=DPI, bbox_inches="tight"
    )
    plt.close()
    print("  Saved: weights_heatmap_dimension.pdf/png")


def plot_weights_heatmap_category_dimension(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
    ...
    fig, axes = plt.subplots(
        n_rows,
        n_cols,
        figsize=(1.5 * SINGLE_COL_WIDTH, 1.5 * SINGLE_COL_WIDTH),
        squeeze=False,
        sharex=True,
        sharey=True,
    ...
    for idx, dim in enumerate(dimensions):
        ...
        summary = (
            subset.groupby(["category", "operator_short"])["value"].mean().reset_index()
        )
        pivot = summary.pivot(
            index="operator_short", columns="category", values="value"
        )
        ...
        show_cbar = False  # (idx % n_cols == n_cols - 1)
    ...
    plt.savefig(
        output_dir / "weights_heatmap_category_dimension.pdf",
        dpi=DPI,
        bbox_inches="tight",
    )
    plt.savefig(
        output_dir / "weights_heatmap_category_dimension.png",
        dpi=DPI,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_operator_usage_distribution(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
...
def plot_weights_per_function(df: pd.DataFrame, operators: list, output_dir: Path):
    ...
    summary = (
        melted.groupby(["problem_id", "operator_short"])["value"].mean().reset_index()
    )
...
def plot_top_operators_by_category(df: pd.DataFrame, operators: list, output_dir: Path):
    ...
    fig, axes = plt.subplots(
        1, len(categories), figsize=(DOUBLE_COL_WIDTH, 2.2), sharey=True
    )

    for ax, cat in zip(axes, categories):
        subset = melted[melted["category"] == cat]
        summary = (
            subset.groupby("operator_short")["value"]
            .mean()
        ...
        short_cat = cat.replace("Low/Moderate ", "L/M ").replace(
            "Multi-modal Weak Structure", "MM Weak"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "top_operators_by_category.pdf", dpi=DPI, bbox_inches="tight"
    )
    plt.savefig(
        output_dir / "top_operators_by_category.png", dpi=DPI, bbox_inches="tight"
    )
    plt.close()
    print("  Saved: top_operators_by_category.pdf/png")


def plot_weight_vs_success_correlation(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
    ...
    weight_agg = (
        weight_melted.groupby("operator_short")["value"]
        .agg(["mean", "std"])
        .reset_index()
    )
    weight_agg.columns = ["operator_short", "mean_weight", "std_weight"]
    success_agg = (
        success_melted.groupby("operator_short")["value"]
        .agg(["mean", "std"])
        .reset_index()
    )
    ...
    shapes = [
        "o" if OPERATOR_TYPE.get(op) == "Exploitation" else "s"
        for op in merged["operator_short"]
    ]
...
def plot_performance_summary(df: pd.DataFrame, output_dir: Path):
    ...
    """
    fig, axes = plt.subplots(
        1, 2, figsize=(1.5 * SINGLE_COL_WIDTH, SINGLE_COL_WIDTH / 2.0), sharey=False
    )
...
def plot_dimension_scaling(df: pd.DataFrame, operators: list, output_dir: Path):
    ...
    # Legend at top, outside the plot
    ax.legend(
        bbox_to_anchor=(1.02, 1), loc="upper left", ncol=1, frameon=False, fontsize=7
    )
...
def plot_operator_ranking_by_dimension(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
...
def plot_weights_by_dimension_boxswarm(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "weights_by_dimension_boxswarm.pdf", dpi=DPI, bbox_inches="tight"
    )
    plt.savefig(
        output_dir / "weights_by_dimension_boxswarm.png", dpi=DPI, bbox_inches="tight"
    )
    plt.close()
    print("  Saved: weights_by_dimension_boxswarm.pdf/png")


def plot_weights_by_category_boxswarm(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "weights_by_category_boxswarm.pdf", dpi=DPI, bbox_inches="tight"
    )
    plt.savefig(
        output_dir / "weights_by_category_boxswarm.png", dpi=DPI, bbox_inches="tight"
    )
    plt.close()
    print("  Saved: weights_by_category_boxswarm.pdf/png")


def plot_success_rate_by_dimension_boxswarm(
    df: pd.DataFrame, operators: list, output_dir: Path
):
    """
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "success_by_dimension_boxswarm.pdf", dpi=DPI, bbox_inches="tight"
    )
    plt.savefig(
        output_dir / "success_by_dimension_boxswarm.png", dpi=DPI, bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate paper-ready plots from NEVO benchmark results"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
